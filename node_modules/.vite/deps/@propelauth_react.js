...
// node_modules/@propelauth/react/dist/index.esm.js
...
var OrgRoleStructure = function(OrgRoleStructure2) {
...
var UserClass = class _UserClass {
  ...
  static fromJSON(json) {
    ...
    try {
      ...
      console.error("Unable to parse User. Make sure the JSON string is a stringified `UserClass` type.", e2);
      throw e2;
    }
  }
};
var OrgMemberInfoClass = class _OrgMemberInfoClass {
  ...
  static fromJSON(json) {
    const obj = JSON.parse(json);
    try {
      return new _OrgMemberInfoClass(obj.orgId, obj.orgName, obj.orgMetadata, obj.urlSafeOrgName, obj.userAssignedRole, obj.userInheritedRolesPlusCurrentRole, obj.userPermissions, obj.orgRoleStructure, obj.userAssignedAdditionalRoles, obj.legacyOrgId);
    } catch (e2) {
      console.error("Unable to parse UserOrgInfo. Make sure the JSON string is a stringified `UserOrgInfo` type.", e2);
      throw e2;
    }
  }
};
function convertOrgIdToOrgMemberInfo(orgIdToOrgMemberInfo) {
  if (orgIdToOrgMemberInfo === void 0) {
    return void 0;
  }
  const orgIdToUserOrgInfo = {};
  for (const orgMemberInfo of Object.values(orgIdToOrgMemberInfo)) {
...
function parseResponse(res) {
  return res.text().then((httpResponse) => {
    try {
      ...
      console.error("Unable to process authentication response", e2);
      return Promise.reject({
        status: 500,
        message: "Unable to process authentication response"
      });
...
var runWithRetriesOnAnyError = async (fn) => {
  return runWithRetriesInner(fn, DEFAULT_RETRIES);
};
var runWithRetriesInner = async (fn, numRetriesLeft) => {
  try {
    return await fn();
  } catch (e2) {
    if (numRetriesLeft <= 0) {
...
var numRetriesLeftToDelay = (numRetriesLeft) => {
  if (numRetriesLeft >= 3) {
    return 100;
...
var LOGGED_IN_AT_KEY = "__PROPEL_AUTH_LOGGED_IN_AT";
var LOGGED_OUT_AT_KEY = "__PROPEL_AUTH_LOGGED_OUT_AT";
var AUTH_TOKEN_REFRESH_BEFORE_EXPIRATION_SECONDS = 10 * 60;
var DEFAULT_MIN_SECONDS_BEFORE_REFRESH = 60 * 2;
var ACTIVE_ORG_ACCESS_TOKEN_REFRESH_EXPIRATION_SECONDS = 60 * 5;
...
function validateAndCleanupOptions(authOptions) {
  try {
    ...
    console.error("Invalid authUrl", e2);
    throw new Error("Unable to initialize auth client");
  }
  if (authOptions.enableBackgroundTokenRefresh === void 0) {
...
function createClient(authOptions) {
  ...
  if (hasWindow()) {
    ...
    if (authOptions.enableBackgroundTokenRefresh) {
      ...
      clientState.refreshInterval = window.setInterval(client.getAuthenticationInfoOrNull, 6e4);
    }
  }
  return client;
}
var DEPRECATED_ORG_SELECTION_LOCAL_STORAGE_KEY = "__last_selected_org";
...
var useClientRef = (props) => {
  const [accessTokenChangeCounter, setAccessTokenChangeCounter] = (0, import_react.useState)(0);
  const {
    authUrl,
  ...
  if (clientRef.current === null) {
    ...
    client.addAccessTokenChangeObserver(() => setAccessTokenChangeCounter((x2) => x2 + 1));
    clientRef.current = {
      authUrl,
      client
    };
  ...
  (0, import_react.useEffect)(() => {
    if (clientRef.current === null) {
      ...
      newClient.addAccessTokenChangeObserver(() => setAccessTokenChangeCounter((x2) => x2 + 1));
      clientRef.current = {
        authUrl,
        client: newClient
      };
...
var useClientRefCallback = (clientRef, callback) => {
  return (0, import_react.useCallback)((...inputs) => {
    var _a;
    const client = (_a = clientRef.current) == null ? void 0 : _a.client;
    if (!client) {
...
var AuthProvider = (props) => {
  ...
  const [authInfoState, dispatch] = (0, import_react.useReducer)(authInfoStateReducer, initialAuthInfoState);
  const {
    clientRef,
    accessTokenChangeCounter
  } = useClientRef({
  ...
  const logout2 = useClientRefCallback(clientRef, (client) => client.logout);
  ...
  const getAccessToken = useClientRefCallback(clientRef, (client) => {
    return async () => {
      const authInfo = await client.getAuthenticationInfoOrNull();
      return authInfo == null ? void 0 : authInfo.accessToken;
    };
  });
  const refreshAuthInfo = (0, import_react.useCallback)(async () => {
    if (clientRef.current === null) {
      return;
    }
  ...
  return import_react.default.createElement(AuthContext.Provider, {
    value
  }, children);
};
var AuthProviderForTesting = ({
  loading,
  userInformation,
  activeOrgFn,
  authUrl,
  ...
  const getAccessTokenForOrg = (0, import_react.useCallback)((orgId) => {
    if (userInformation == null ? void 0 : userInformation.getAccessTokenForOrg) {
  ...
  }, [userInformation == null ? void 0 : userInformation.getAccessTokenForOrg]);
  const contextValue = {
    loading: !!loading,
    authInfo,
    ...
    tokens: {
      getAccessTokenForOrg,
      getAccessToken: () => Promise.resolve((userInformation == null ? void 0 : userInformation.accessToken) ?? "ACCESS_TOKEN")
    }
  };
  return import_react.default.createElement(AuthContext.Provider, {
    value: contextValue
...
function getAuthInfoForTesting(userInformation) {
  ...
  const accessTokenWithDefault = userInformation.accessToken === void 0 ? "PLACEHOLDER_ACCESS_TOKEN" : userInformation.accessToken;
  return {
...
function useAuthInfo() {
  ...
  if (loading) {
    return {
      ...
      impersonatorUserId: void 0,
      refreshAuthInfo,
      tokens,
      accessTokenExpiresAtSeconds: void 0
    };
...
function RedirectToSignup({
  ...
  (0, import_react.useEffect)(() => redirectToSignupPage({
    postSignupRedirectUrl,
    userSignupQueryParameters
  }), []);
...
function withRequiredAuthInfo(Component, args) {
  ...
  const WithRequiredAuthInfoWrapper = (props) => {
    ...
    function displayLoading() {
      if (args == null ? void 0 : args.displayWhileLoading) {
    ...
    function displayLoggedOut() {
      if (args == null ? void 0 : args.displayIfLoggedOut) {
        return args.displayIfLoggedOut;
      } else if (defaultDisplayIfLoggedOut) {
        return defaultDisplayIfLoggedOut;
      }
      return import_react.default.createElement(RedirectToLogin, null);
    }
    if (loading) {
      ...
      return import_react.default.createElement(Component, loggedInProps);
    } else {
...
var RequiredAuthWrappedComponent = withRequiredAuthInfo(({
  children
}) => import_react.default.createElement(import_react.default.Fragment, null, children));
var RequiredAuthProvider = (props) => {
  const {
    children,
    displayIfLoggedOut,
    displayWhileLoading,
  ...
  return import_react.default.createElement(AuthProvider, _extends({}, sharedProps, {
    defaultDisplayIfLoggedOut: displayIfLoggedOut,
    defaultDisplayWhileLoading: displayWhileLoading
  }), import_react.default.createElement(RequiredAuthWrappedComponent, null, children));
};
function withAuthInfo(Component, args) {
  ...
  const WithAuthInfoWrapper = (props) => {
    ...
    if (loading) {
      ...
      return import_react.default.createElement(Component, notLoggedInProps);
    }
  };
...
@propelauth/react/dist/index.esm.js:
...
@propelauth/react/dist/index.esm.js:
   ...
   *)
*/
//# sourceMappingURL=@propelauth_react.js.map
